        ...
        success: function(data) {
            ...
            if (data.status == 1) {
                for (var i = 0; i < data.data.length; i++) {
                    sessionStorage.setItem("cart_totalItems", data.data.length);
                    sessionStorage.setItem("cart_product_variant_combination_id" + i, data.data[i].product_variant_combination_id);
                    sessionStorage.setItem("cart_name" + i, data.data[i].product_combinations.product.name);
...
function razorpay(amount, order_id, mobile_no, email_id, name) {
// alert(order_id);
// return;

    var options = {
        // "key": "rzp_test_u98BYGIVDoFxzJ",
        "key": "rzp_live_StQqpGuHYEtG81", // Enter the Key ID generated from the Dashboard    
        "amount": amount * 100,
        "currency": "INR",
        "name": "Clipshoppers",
      ...
      "order_id": order_id, //This is a sample Order ID. Create an Order using Orders API. (https://razorpay.com/docs/payment-gateway/orders/integration/#step-1-create-an-order). Refer the Checkout form table given below
        ...
        form.append("razorpay_signature", response.razorpay_signature);
            if (response.razorpay_payment_id) {
                $.ajax({
                    url: BASE_URL + "orders/verify_payment",
                    type: "POST",
                    dataType: 'json',
                    crossDomain: true,
                    processData: false,
...
function placeOrder() {
    ...
    form.append("payment_method_id", payment_type_id);
        ...
        success: function(data) {
            ...
            if (data.status == 1 && data.data !== "") {
                ...
                if (payment_type_id == 1) {
                    ...
                    var order_id = data.data.transaction_id;
                    var name = localStorage.getItem("user_oilkart_name");
                    var mobile_no = localStorage.getItem("user_oilkart_phone");
                    var email_id = localStorage.getItem("user_oilkart_email");

                    // alert(order_id);
                    razorpay(amount, order_id, mobile_no, email_id, name);
                    // console.log(data);
